...
$cookiedir = ''; if (dirname($_SERVER['SCRIPT_NAME']) != '/') {
...
if (!is_writable(realpath(dirname(__FILE__)))) {
    die('<pre>ERROR: Shaarli does not have the right to write in its own directory ('.realpath(dirname(__FILE__)).').</pre>');
}
...
function showATOM()
{
    ...
    while ($i < $nblinksToDisplay && $i < count($keys)) {
        ...
        // If user wants permalinks first, put the final link in description
        if ($usepermalinks === true) {
            $descriptionlink = htmlspecialchars('(<a href="'.$absurl.'">Link</a>)');
        }
        if (strlen($link['description']) > 0) {
            $descriptionlink = $descriptionlink;
        }
...
function renderPage()
{
    ...
    if (isset($_POST['save_edit'])) {
        ...
        if (strstr($returnurl, 'do=addlink')) {
            $returnurl = '?';
        } //if we come from ?do=addlink, set returnurl to homepage instead
    ...
    if (isset($_GET['post'])) {
        ...
        if (!$link) {
            ...
            if ($url == '') {
                // In case of empty URL, this is just a text (with a link that points to itself)
                ...
                if ($link['url'] != '') {
                    ...
                    if ($dblink == false) {  // Link not in database, let's import it...
                       ...
                       $link['linkdate'] = date('Ymd_His', $raw_add_date);
                        ...
                        // link already present in database.
...
function computeThumbnail($url, $href = false)
{
    ...
    if ($domain == 'youtube.com' || $domain == 'www.youtube.com') {
        ...
        if (!empty($params['v'])) {
            return array('src' => 'https://img.youtube.com/vi/'.$params['v'].'/default.jpg',
    ...
    if ($domain == 'pix.toile-libre.org') {
        ...
        if (!empty($params) && !empty($params['img'])) {
            return array('src' => 'http://pix.toile-libre.org/upload/thumb/'.urlencode($params['img']),
    ...
    if ($domain == 'imgur.com') {
        ...
        if (startsWith($path, '/r/')) {
            return array('src' => 'https://i.imgur.com/'.basename($path).'s.jpg',
                                                  'href' => $href, 'width' => '90', 'height' => '90', 'alt' => 'imgur.com thumbnail', );
        }
        if (startsWith($path, '/gallery/')) {
            return array('src' => 'https://i.imgur.com'.substr($path, 8).'s.jpg',
                                                        'href' => $href, 'width' => '90', 'height' => '90', 'alt' => 'imgur.com thumbnail', );
        }

        if (substr_count($path, '/') == 1) {
            return array('src' => 'https://i.imgur.com/'.substr($path, 1).'s.jpg',
                                                     'href' => $href, 'width' => '90', 'height' => '90', 'alt' => 'imgur.com thumbnail', );
        }
    }
    if ($domain == 'i.imgur.com') {
        ...
        if (!empty($pi['filename'])) {
            return array('src' => 'https://i.imgur.com/'.$pi['filename'].'s.jpg',
                                                  'href' => $href, 'width' => '90', 'height' => '90', 'alt' => 'imgur.com thumbnail', );
        }
    }
    ...
    if ($domain == 'flickr.com' || endsWith($domain, '.flickr.com')
        ...
        if ($domain == 'vimeo.com') {   // Make sure this vimeo url points to a video (/xxx... where xxx is numeric)
            ...
            if (!preg_match('!/\d+.+?!', $path)) {
                return array();
            } // This is not a single video URL.
        }
...
function templateTZform($ptz = false)
{
    if (function_exists('timezone_identifiers_list')) {
        // because of old php version (5.1) which can be found on free.fr
...
function writeConfig()
{
    if (is_file($GLOBALS['config']['CONFIG_FILE']) && !isLoggedIn()) {
        die('You are not authorized to alter config.');
